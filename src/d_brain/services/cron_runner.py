...
async def run_cron(settings: Any, bot: Any) -> None:
    ...
    async def deliver(chat_id: int, text: str) -> None:
        # HTML first; fall back to plain text on a Telegram parse error, the
        # same contract the shell send_telegram uses for reports.
        try:
            await bot.send_message(chat_id, text, parse_mode="HTML")
        except Exception:  # noqa: BLE001 — bad markup must still deliver
            import re
