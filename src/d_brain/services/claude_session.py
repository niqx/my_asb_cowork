...
DEFAULT_STALL_TIMEOUT = 300  # no active turn for this long ⇒ wedged (5 min,
# generous so a slow tool-heavy turn is never mistaken for a stall)
DEFAULT_MAX_RESUBMITS = 2  # on a stall, re-send the prompt up to twice
...
class ClaudeSession:
    ...
    def __init__(
        ...
        *,
        ...
        self.model = model
        # Extra env vars (e.g. TODOIST_API_KEY) baked into the tmux start
        # command so the persistent `claude` inherits them regardless of which
        # process (bot / pipeline / watchdog) first spawns the session.
        self.env_prefix = dict(env_prefix) if env_prefix else {}
    ...
    def _capture(self) -> str:
        # -J joins soft-wrapped rows: the pane is fixed-width, so a long line
        # of model output (a 700-char JSON string, an HTML report) wraps at the
        # column limit and capture WITHOUT -J inserts a newline at every wrap
        # point — landing literal control chars inside JSON strings and
        # breaking the parse. -J rejoins them; real line breaks and the
        # line-anchored <<<R/E>>> markers are preserved.
        return self._tmux(
            "capture-pane", "-t", self._target, "-p", "-J", "-S", _CAPTURE_SCROLLBACK
        ).stdout
    ...
    def _start_command(self) -> str:
        env_parts = [
            f"{k}={shlex.quote(v)}" for k, v in self.env_prefix.items() if v
        ]
        parts = [*env_parts, shlex.quote(self.claude_bin), "--dangerously-skip-permissions"]
        if self.mcp_config:
    ...
    def _ensure_submitted(self, prompt: str, rid: str, *, wrap: bool) -> None:
    ...
    def ask(
        ...
        *,
        ...
        with self._locked() as got:
            ...
            self._send_prompt(prompt, rid, wrap=wrap)
            self._ensure_submitted(prompt, rid, wrap=wrap)
            ...
            while self._clock() < deadline:
                ...
                if is_working(cap) or not input_has_text(cap):
                    ...
                    # certainly never submitted (a cold/long-idle pane swallows
                    # the first keystrokes). Clear the input and re-send once on
                    # the now-awake pane before declaring the session wedged —
                    # this is the common unattended-cron failure (morning brief).
                    if resubmits < self._max_resubmits:
                        resubmits += 1
                        logger.warning(
                            "%s: no active turn after %.0fs — resubmitting (%d/%d)",
                            log_id, self._stall_timeout, resubmits, self._max_resubmits,
                        )
                        self._interrupt()
                        self._sleep(self._paste_settle)
                        self._send_prompt(prompt, rid, wrap=wrap)
